...
void file_openFile()
{
	...
	string fileNameData = getArrayIndex(getVariable(ARGUMENT_VARIABLE), 0);
	string fileName = convertToPrintableStr(fileNameData);
	Object* file = createObject();
	
	// Step 2: Open file
	std::fstream* fileStream = new std::fstream{ std::ios::in | std::ios::out | std::ios::binary };
...
void file_getFileContent()
{
	...
	ulonger len = getPrimitiveLength(getVariable(ARGUMENT_VARIABLE)) > 1 ? stringToUI64(getArrayIndex(getVariable(ARGUMENT_VARIABLE), 1).substr(1)) : ULLONG_MAX;
...
void file_writeFileContent()
{
	...
	Object* fileObject = retrieveObjectFromString(fileVar);
	std::fstream* file = reinterpret_cast<std::fstream*>(stringToUI64(getObjectProperty(fileObject, string("_file")).substr(1)));
	string insert = convertToProperStr(getArrayIndex(getVariable(ARGUMENT_VARIABLE), 1).substr(1));
	*file << insert;
}